...
export default function GenerarCuotaClubButton({ athletes }: { athletes: Athlete[] }) {
  ...
  async function handleGenerar() {
    ...
    await Promise.all(athletes.map(a =>
      supabase.from('payments').insert({
        athlete_id: a.id,
        concept: `Cuota WeAthletics - ${monthName}`,
  ...
  if (!open) return (
    <button onClick={() => setOpen(true)} style={{ padding: '9px 18px', borderRadius: '9px', background: 'linear-gradient(135deg,#1E2A5E,#4BA3D9)', color: 'white', fontSize: '13px', fontWeight: '600', border: 'none', cursor: 'pointer' }}>
      {done ? '✓ Cuotas generadas' : '€ Generar cuotas del mes'}
    </button>
  )
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, backgroundColor: 'rgba(0,0,0,0.7)', zIndex: 100, display: 'flex', alignItems: 'center', justifyContent: 'center' }} onClick={() => setOpen(false)}>
      <div style={{ backgroundColor: '#0A0E1A', border: '1px solid rgba(75,163,217,0.15)', borderRadius: '16px', padding: '24px', width: '400px' }} onClick={e => e.stopPropagation()}>
        <div style={{ color: '#CDD0E0', fontSize: '15px', fontWeight: '700', marginBottom: '6px' }}>Generar cuotas del mes</div>
        <div style={{ color: '#3A4A70', fontSize: '12px', marginBottom: '18px' }}>Se creará un pago para los {athletes.length} atletas activos.</div>
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '12px', marginBottom: '16px' }}>
          ...
          <div>
            <label style={{ color: '#3A4A70', fontSize: '11px', fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.08em', display: 'block', marginBottom: '6px' }}>Importe (€)</label>
            <input type="number" value={amount} onChange={e => setAmount(e.target.value)} min="1" step="0.01" style={{ width: '100%', backgroundColor: 'rgba(75,163,217,0.05)', border: '1px solid rgba(75,163,217,0.15)', borderRadius: '9px', padding: '10px 14px', color: '#E8EAF0', fontSize: '14px', outline: 'none', boxSizing: 'border-box', colorScheme: 'dark' }} />
          </div>
        </div>
        <div style={{ display: 'flex', gap: '8px' }}>
          ...
          <button onClick={handleGenerar} disabled={loading || !amount} style={{ flex: 2, padding: '10px', borderRadius: '9px', background: 'linear-gradient(135deg,#1E2A5E,#4BA3D9)', border: 'none', color: 'white', fontSize: '13px', fontWeight: '700', cursor: 'pointer' }}>
            {loading ? 'Generando...' : `Generar ${athletes.length} cuotas`}
          </button>
        </div>
      </div>
    </div>
